...
var jLoading = {
  ...
  show: function(overDomId) {
    if (!overDomId) {
      overDomId = "body";
    }
    if (this.list[overDomId]) {
      this.list[overDomId] += 1;
    } else {
      this.list[overDomId] = 1;
      $(overDomId).loading(this.option);
    }
  },
  stop: function(overDomId) {
    if (!overDomId) {
      overDomId = "body";
    }
    if (this.list[overDomId]) {
    ...
    if (!this.list[overDomId]) {
      delete this.list[overDomId];
      $(overDomId).loading("stop");
    }
  }
};